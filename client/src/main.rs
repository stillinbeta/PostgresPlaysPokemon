...
fn get_arg_radx(matches: &clap::ArgMatches, key: &str, radix: u32) -> Option<u32> {
    matches
        .value_of(key)
        .map(|v| u32::from_str_radix(v, radix).expect(&format!("failed to parse \"{}\"", key)))
}
...
fn main() {
    let matches = app_from_crate!()
        ...
        .subcommand(
            SubCommand::with_name("set-memory")
                .arg(Arg::with_name("DESTINATION").required(true).index(1))
                .arg(Arg::with_name("VALUE").required(true).index(2)),
        )
        .subcommand(
            SubCommand::with_name("update-pokemon")
                .arg(Arg::with_name("SLOT").required(true).index(1))
                .arg(make_arg!("id"))
                .arg(make_arg!("hp"))
                .arg(make_arg!("level"))
                .arg(make_arg!("max-hp"))
        ...
        .subcommand(
            SubCommand::with_name("update-item")
                .arg(Arg::with_name("SLOT").required(true).index(1))
                .arg(make_required_arg!("id"))
                .arg(make_required_arg!("quantity")),
        )
        .subcommand(
            SubCommand::with_name("add-item")
                .arg(make_required_arg!("id"))
                .arg(make_required_arg!("quantity")),
        )
        .subcommand(
    ...
    match matches.subcommand_name() {
        ...
        Some("set-memory") => {
            ...
            let dest = u32::from_str_radix(matches.value_of("DESTINATION").unwrap(), 16)
                .expect("Failed to parse DESTINATION");
            let val = u32::from_str_radix(matches.value_of("VALUE").unwrap(), 16)
        ...
        Some("update-pokemon") => {
            ...
            let resp = client.set_pokemon(&ppp_client::UpdatePokemon {
